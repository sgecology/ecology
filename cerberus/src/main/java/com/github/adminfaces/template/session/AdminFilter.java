...
public class AdminFilter implements Filter {
    ...
    public void init(FilterConfig filterConfig) {
        if(adminConfig == null || adminSession==null) {
            initBeans();
        }
    ...
    private void initBeans() {
        ...
        try {
            ...
            //adminConfig = new AdminConfig(); //CDI.current().select(AdminConfig.class).get();
            if (null==adminSession) {
              adminSession = new AdminSession();//CDI.current().select(AdminSession.class).get();
            }
