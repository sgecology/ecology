...
import net.ecology.encryption.SimpleEncryptor;
 ...
 */
@Component
public class ResourcesServiceHelper {
	@Inject
		...
		try {
			...
			for (String key :secretKeyMap.keySet()) {
				archivedConfig.addSubordinate(Configuration.builder()
						...
						.value(SimpleEncryptor.encode(secretKeyMap.get(key)))
						.valueExtended(key)
						.build())
				;
			}
	...
	public static Attachment buidAttachment(final File file) throws CerberusException {
		...
		try {
			attachment = Attachment.builder()
					.name(file.getName())
					.data(SystemIOUtility.toByteArray(new FileInputStream(file)))
	...
	public Attachment buildAttachment(final String resourceName, final Context container, String encryptionKey) throws CerberusException {
		Attachment attachment = null;
		int lastDot = resourceName.lastIndexOf(CommonConstants.FILE_EXTENSION_SEPARATOR);
		...
		try {
			if (CommonUtility.isNotEmpty(encryptionKey))
				procEncyptionKey = SimpleEncryptor.encode(encryptionKey);

			attachment = Attachment.builder()
					.name(resourceName)
					.data(SystemIOUtility.toByteArray(container.get(resourceName)))
					.mimetype(MimeTypes.getMimeType(fileExtension))
	...
	public Collection<Attachment> buildAttachments(final Context ioContainer) throws CerberusException {
		int fileExtPos;
		...
		try {
			...
			for (Object key :ioContainer.keys()) {
				procEncyptionKey = (CommonUtility.isNotEmpty(ioContainer.getSecret(key))) ? SimpleEncryptor.encode(ioContainer.getSecret(key)):null;

				fileExtPos = ((String)key).lastIndexOf(CommonConstants.FILE_EXTENSION_SEPARATOR);
				fileExtension = ((String)key).substring(fileExtPos+1);

				attachments.add(Attachment.builder()
						.name((String)key)
						.data(SystemIOUtility.toByteArray(ioContainer.get(key)))
						.mimetype(MimeTypes.getMimeType(fileExtension))
						.encryptionKey(procEncyptionKey)
						.build()
				);
	...
	/*public Collection<Attachment> buildAttachments(final IOContext ioContainer) throws CerberusException {
		int fileExtPos;
		String fileExtension = null;
		String procEncyptionKey = null;
		Collection<Attachment> attachments = CollectionsUtility.newCollection(); 
		try {
			...
			for (Object key :ioContainer.keys()) {
				...
				attachments.add(Attachment.builder()
						.name((String)key)
						.data(IOUtils.toByteArray(ioContainer.get(key)))
						.mimetype(MimeTypes.getMimeType(fileExtension))
						.encryptionKey(procEncyptionKey)
						.build()
				);
